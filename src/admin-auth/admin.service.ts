...
import { sendOtp } from 'src/utils/email.service';
import { generateOtp } from 'src/utils/otp-generator';
import { ChangePasswordDto } from './dto/changepassword.dto';
import { RefreshTokenDto } from './dto/refresh-token.dto';
import {AccessTokenResponseDto} from './dto/login-response.dto';
...
async signup(signupAdminDto: SignupAdminDto) {
  ...
  async login(loginAdminDto: LoginAdminDto) {
    ...
    const tokens = await this.grpcClientService.getToken(userId, deviceId, role , email
      // // email: admin.email,
      // deviceId: deviceId,
      // role: 'admin',
      // userId: uniqueUserId 
 ...
 console.log("otp is being generated ");
  ...
  // 1. Validate the token first (optional, if needed)
  // const validation = await this.validateToken(accessToken);
  // if (!validation.isValid) {
  //   throw new UnauthorizedException('Invalid token');
  // }

  // 2. Extract `userId` and `deviceId` from the token (if needed)
  // (Assuming your gRPC service handles token invalidation)
